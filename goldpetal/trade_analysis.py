...
WHAT_IT_GUESSES = {
    ...
    "S5_MINEDGE": "Depth imbalance vs expected move. Waits until expected points can cover Angel fees (S5_COVER_FEES), even when IGNORE_FEES is on. Delivery — holds overnight. Only S16 flattens at MARKET_CLOSE.",
    "S6_MIN30": "Depth imbalance vs a 30-point expected-move floor.",
    "FLOW_BRAIN": "Tick LTP + TBQ + TSQ pressure plus desk gates from S5 fee-cover, S8 NET/IMB/book-drop, S16/S19 1m structure, S20 fade-block. Next-move lab ranks whether those states precede 5s/30s/1m/5m moves after charges before any GBDT/RF. Paper-wired, ENABLE_FLOW_BRAIN defaults false. Not S7_HOURLY. Not S16. Not live.",
    "S8_NET_ZIGZAG": "Order-book NET zigzags. Needs a strong rising imbalance (default 14%) and a target that can cover fees. Delivery — holds overnight. Only S16 flattens at MARKET_CLOSE.",
    ...
    "S16_HHHL_WICK_1H": "Wait for the 1h candle to finish. Up close: HH+green LONG / LL+red SHORT. Down close: wick (gap 0) — upper wick SHORT, lower wick BUY. FLIP at that close. Mood and market regime are observe-only. Intraday only: flatten at MARKET_CLOSE, leftover at next open. Never overnight.",
    "S18_OHLC_VOL_HTF": "Wait for the 1h to finish. Base AND: green/red + close vs prev + HH/LL + volume up + close vs yesterday. Learner may change that pack from ticks (OHLC, volume, TBQ/TSQ, n_ticks). FLIP. Delivery — holds overnight. Only S16 flattens at MARKET_CLOSE. Live-eligible — tick Lots or ₹, then you Arm live.",
    "S19_BODY_CLOSE_1H": "Wait for the 1h to finish. LONG = green and C>prevC. SHORT = red and C<prevC. Mixed / doji / equal close: hold. FLIP only on the opposite aligned hour. Delivery — holds overnight. Only S16 flattens at MARKET_CLOSE. Live-eligible — tick Lots or ₹, then you Arm live.",
